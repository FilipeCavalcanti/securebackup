...
void setup()
{
  ...
  Serial.println("-- Sensoriamento v0.1 --");
...
void gyro()
{
  Serial.print("\nInicializando Gyro/Acelerometro...");
...
void loop()
{
  ...
  */
  //Serial.println("Data; Hora; Velocidade; Latitude; Longitude; Altitude");
  String dados = "";
  //escreve dados do gps
  dados += (gps.date.value()); dados += (";"); dados += (gps.time.value()); dados += (";");  dados += (gps.speed.kmph()); dados += (";");  dados += (gps.location.lat());
  dados += (gps.location.lng()); dados += (";");  dados += (gps.altitude.meters()); dados += (";");  dados += (gps.satellites.value()); dados += (";"); 
  //escreve dados do gyro
  dados += (Xg); dados += (";"); dados += (Yg); dados += (";"); dados += (Zg); dados += (";");
  dados += (x); dados += (";"); dados += (y); dados += (";"); dados += (z);

  Serial.println(dados);

  File dataFile = SD.open("testeprot2.txt", FILE_WRITE);
